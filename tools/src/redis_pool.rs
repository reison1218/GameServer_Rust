use super::*;
use redis::{transaction, Client, Commands, Connection, Pipeline, RedisResult, Value, FromRedisValue};
...
impl RedisPoolTool {
    ...
    pub fn init(add:&str,password:&str) -> RedisPoolTool {
        let mut client = redis::Client::open(add).unwrap();
        info!("初始化redis客户端完成!");
        let mut redis_pool = RedisPoolTool {
            client: client.clone(),
            conn: client.get_connection().unwrap(),
        };
        redis::pipe().cmd("AUTH").arg(password).execute(&mut redis_pool.conn);
        redis_pool
    }

    ///操作hash数据结构
    pub fn hset<T: FromRedisValue>(&mut self, index:u32,hkey: &str, key: &str, value: &str)-> Option<T>{
    ...
    pub fn hget<T: FromRedisValue>(&mut self, index:u32,hkey: &str, key: &str) -> Option<T> {
        ...
        get_pip().cmd("select").arg(0).execute(&mut self.conn);
        if res.is_err(){
            error!("hget has error:{:?},key:{:?}",res.err().unwrap(),hkey);
            return None;
        }
        Some(res.unwrap())
    }
    ...
    /// key：有序集合成员的key
    /// value：有序集合成员value
    pub fn zadd<T: FromRedisValue>(&mut self,index:u32,zkey: &str, key: &str, value: isize) -> Option<T> {
        get_pip().cmd("select").arg(index).execute(&mut self.conn);
...
pub fn test_api(add:&str,pass:&str) {
    ...
    let mut value:Option<Vec<u32>> = rpt.hget(1,"uid_2_pid","1011000001");
     if value.is_some() {
