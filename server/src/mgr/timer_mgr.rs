...
pub fn init(gm: Arc<RwLock<GameMgr>>){
    ...
    //每5分钟保存玩家数据
    save_timer(gm.clone());
    info!("定时任务初始化完毕!耗时:{:?}ms",time.elapsed().unwrap().as_millis())
}
...
fn zero_day(gm: Arc<RwLock<GameMgr>>){
    ...
    let zero_day = move||{
        loop{
            ...
            let next_time = now_time.with_hour(23).unwrap().with_minute(59).unwrap().with_second(59).unwrap();
            ...
            info!("零点重制完成！耗时{:?}ms",now_time.elapsed().unwrap().as_millis());
        }
    };
    ...
    let re = move || loop {
        ...
        match res {
            ...
            Ok(mut vec) => {
                ...
                info!("执行定时保存玩家结束，执行数量:{},耗时:{}ms",count,time.elapsed().unwrap().as_millis());
            }
        }
    };
    std::thread::spawn(m);
