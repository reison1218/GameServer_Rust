...
pub struct WebSocketHandler {
    ...
    pub gm: Arc<Mutex<GameMgr>>, //gamemgr的引用计数器
}
...
impl WebSocketHandler {
    ...
    fn handle_binary(&mut self, bytes: &[u8]) {
        ...
        if packet.get_cmd() == C_USER_LOGIN.value() as u32 {
            ...
            {
                let mut gm: MutexGuard<GameMgr> = self.gm.lock().unwrap();
...
///封装会话函数
fn insert_channel(ws: Arc<WsSender>, gm: MutexGuard<GameMgr>, user_id: u32) {
    let mut gm = gm;
    let sender = ws;
