...
use mysql::{Value, QueryResult, Error};
...
pub trait Entity: Send+Sync{
    ...
    fn init(user_id: u32, tem_id: Option<u32>, js: JsonValue) -> Self where Self: Sized ;
}
...
pub trait Dao: Entity {
    ...
    fn update(&mut self) -> Result<u32, String>{
        ...
        let mut qr:Result<QueryResult,Error> = DB_POOL.exe_sql(
            sql.as_str(),
            Some(v),
        );
        if qr.is_err() {
    ...
    fn insert(&mut self) -> Result<u32, String>{
        ...
        match tem_id {
            Some(tem_id)=>{
                sql.push_str(" values(:user_id,tem_id:tem_id,:content)");
            },
            None=>{
