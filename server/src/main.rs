...
fn main() {
    ...
    //初始化websocket
    //init_websocket(game_mgr.clone());

    //初始化tcpserver
    let init_tcp = task::spawn(init_tcpserver(game_mgr.clone()));
    block_on(init_tcp);
    info!(
        "服务器启动完成，监听端口：{},耗时：{}ms",
        9999,
...
///初始化tcp服务端
async fn init_tcpserver(game_mgr: Arc<RwLock<GameMgr>>) {
    let t = tcpsocket::new(game_mgr.clone());
    t.await;
}

///初始化websocket
async fn init_websocket(game_mgr: Arc<RwLock<GameMgr>>) {
    //初始化websocket
