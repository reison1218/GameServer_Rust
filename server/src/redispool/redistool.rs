...
impl RedisPoolTool{
    ...
    pub fn init()->RedisPoolTool{
        ...
        RedisPoolTool{client:client.clone(),conn:client.get_connection().unwrap()}
    }

    ///操作hash数据结构
    pub fn set_hset(&mut self,hkey:&str,key:&str,value:&str)-> RedisResult<Value>{
        get_pip().cmd("HSET").arg(hkey).arg(key).arg(value).query(&mut self.conn)
    }

    ///读hash数据结构
    pub fn get_hset(&mut self,hkey:&str,key:&str)-> RedisResult<Value>{
        get_pip().cmd("HGET").arg(hkey).arg(key).query(&mut self.conn)
    }
    ...
    pub fn add_zset(&mut self,zkey:&str,key:&str,value:isize)->RedisResult<Value>{
        get_pip().cmd("ZADD").arg(zkey).arg(value).arg(key).query(&mut self.conn)
    }

    ///得到整个有序集合
    /// zkey:有序集合的key
    pub fn get_zset(&mut self,zkey:&str)->RedisResult<Value>{
        get_pip().cmd("ZRANGE").arg(zkey).arg(0).arg(-1).arg("WITHSCORES").query(&mut self.conn)
    }
    ...
    pub fn zincrby(&mut self,zkey:&str,key:&str)->RedisResult<Value>{
        get_pip().cmd("ZINCRBY").arg(zkey).arg("increment").query(&mut self.conn)
    }
}
 ...
 pub fn test_redis(){
     ...
     r = pip.cmd("HSET").arg("test").arg("key_1").arg("value_1").query(&mut con).unwrap();
 }
