...
fn main() -> anyhow::Result<()> {
    let mut room_cache_array = vec![];
    room_cache_array.push(1);
    room_cache_array.push(2);
    room_cache_array.push(3);
    room_cache_array.push(4);
    room_cache_array.par_sort_by(|a, b| b.cmp(&a));
    println!("{:?}", room_cache_array);
    let res = room_cache_array.get(0).unwrap();
    println!("{}", res);
